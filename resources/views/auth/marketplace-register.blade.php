@extends('layout.master-mini')

@section('content')
    <div class="content-wrapper container auth-wrapper">
        <div class="auth-card">
            <div class="auth-header">
                <div class="auth-logo">
                    <img src="{{ asset('assets/images/logo-w.png') }}" alt="Logo" style="height:32px;">
                </div>
                <div class="auth-title">{{ __('إنشاء حساب') }}</div>
                <div class="auth-subtitle">{{ __('سجّل حساب مشتري لإتمام عمليات الشراء') }}</div>
            </div>
            <div class="auth-body">
                @if ($errors->any())
                    <div class="alert alert-danger">
                        <ul class="mb-0">
                            @foreach ($errors->all() as $e)
                                <li>{{ $e }}</li>
                            @endforeach
                        </ul>
                    </div>
                @endif
                <form method="POST" action="{{ route('marketplace.register.post') }}">
                    @csrf
                    @php($intendedUrl = request()->query('intended'))
                    @if ($intendedUrl && !str_contains($intendedUrl, '/notifications/'))
                        <input type="hidden" name="intended" value="{{ $intendedUrl }}">
                    @endif
                    @php($loggedIn = \Illuminate\Support\Facades\Auth::guard('web')->check())
                    <div class="mb-3">
                        <label class="form-label">{{ __('الاسم الكامل') }}</label>
                        <input type="text" name="name" class="form-control" placeholder="محمد أحمد" required>
                    </div>
                    @if (!$loggedIn)
                        <div class="mb-3">
                            <label class="form-label">{{ __('البريد الإلكتروني') }}</label>
                            <input type="email" name="email" class="form-control" placeholder="name@example.com"
                                required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">{{ __('كلمة المرور') }}</label>
                            <input type="password" name="password" class="form-control" placeholder="••••••" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">{{ __('تأكيد كلمة المرور') }}</label>
                            <input type="password" name="password_confirmation" class="form-control" placeholder="••••••"
                                required>
                        </div>
                    @endif
                    <button type="submit" class="btn btn-auth-primary">{{ __('تسجيل') }}</button>
                </form>

                <div class="text-center my-3">
                    <span class="text-muted">{{ __('أو') }}</span>
                </div>

                <a href="{{ route('auth.google') }}"
                    class="btn btn-outline-dark w-100 d-flex align-items-center justify-content-center">
                    <svg class="me-2" width="18" height="18" xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 48 48">
                        <path fill="#EA4335"
                            d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z" />
                        <path fill="#4285F4"
                            d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z" />
                        <path fill="#FBBC05"
                            d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z" />
                        <path fill="#34A853"
                            d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z" />
                        <path fill="none" d="M0 0h48v48H0z" />
                    </svg>
                    {{ __('التسجيل بواسطة Google') }}
                </a>

                <div class="auth-alt">
                    {{ __('لديك حساب بالفعل؟') }}
                    @php($intendedUrl = request()->query('intended'))
                    <a
                        href="{{ route('marketplace.login', $intendedUrl && !str_contains($intendedUrl, '/notifications/') ? ['intended' => $intendedUrl] : []) }}">{{ __('تسجيل دخول') }}</a>
                </div>
            </div>
        </div>
    </div>
@endsection
