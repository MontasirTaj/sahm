<nav class="sidebar sidebar-offcanvas dynamic-active-class-disabled" id="sidebar">
    <ul class="nav">
        ...
        @if ($isTenantContext && $tenantUser)
                            ...
                            @php
                                ...
                                $tenantAvatarUrl = $tenantAvatarPath
                                    ...
                                    : 'https://ui-avatars.com/api/?name=' .
                                        urlencode($tenantUser->name ?? 'User') .
            ...
            @if ($isManager)
                <li
                    class="nav-item {{ active_class(['tenant.subdomain.users.*', 'tenant.subdomain.roles.*', 'tenant.subdomain.permissions.*']) }}">
                    <a class="nav-link" data-toggle="collapse" href="#tenant-users-menu"
                        aria-expanded="{{ is_active_route(['tenant.subdomain.users.*', 'tenant.subdomain.roles.*', 'tenant.subdomain.permissions.*']) }}"
                        aria-controls="tenant-users-menu">
                        <i class="menu-icon mdi mdi-account-multiple"></i>
                        <span class="menu-title">{{ __('app.users') }}</span>
                        <i class="menu-arrow"></i>
                    </a>
                    <div class="collapse {{ show_class(['tenant.subdomain.users.*', 'tenant.subdomain.roles.*', 'tenant.subdomain.permissions.*']) }}"
                        id="tenant-users-menu">
                        <ul class="nav flex-column sub-menu">
                            <li class="nav-item {{ active_class(['tenant.subdomain.users.*']) }}">
                                <a class="nav-link"
                                    href="{{ route($prefix . '.users.index', ['subdomain' => $subdomain]) }}">{{ __('app.users') }}</a>
                            </li>
                            <li class="nav-item {{ active_class(['tenant.subdomain.roles.*']) }}">
                                <a class="nav-link"
                                    href="{{ route($prefix . '.roles.index', ['subdomain' => $subdomain]) }}">{{ __('app.roles') }}</a>
                            </li>
                            <li class="nav-item {{ active_class(['tenant.subdomain.permissions.*']) }}">
                                <a class="nav-link"
                                    href="{{ route($prefix . '.permissions.index', ['subdomain' => $subdomain]) }}">{{ __('app.permissions') }}</a>
                            </li>
                            <li class="nav-item {{ active_class(['tenant.subdomain.roles.mapping']) }}">
                                <a class="nav-link"
                                    href="{{ route($prefix . '.roles.mapping', ['subdomain' => $subdomain]) }}">
                                    <i class="mdi mdi-link-variant me-1"></i>
                                    <span>{{ __('ربط الأدوار بالصلاحيات') }}</span>
                                </a>
                            </li>
                        </ul>
                    </div>
            ...
            @endif
            @if (
                $tenantUser->can('Add_sahm') ||
                    $tenantUser->can('Update_sahm') ||
                    $tenantUser->can('Delete_sahm') ||
                    $tenantUser->can('manage shares'))
                <li class="nav-item {{ active_class(['tenant.subdomain.shares.*']) }}">
                    <a class="nav-link" data-toggle="collapse" href="#tenant-shares-menu"
                        aria-expanded="{{ is_active_route(['tenant.subdomain.shares.*']) }}"
                        aria-controls="tenant-shares-menu">
                        <i class="menu-icon mdi mdi-office-building"></i>
                        <span class="menu-title">{{ __('عروض الأسهم') }}</span>
                        <i class="menu-arrow"></i>
                    </a>
                    <div class="collapse {{ show_class(['tenant.subdomain.shares.*']) }}" id="tenant-shares-menu">
                        <ul class="nav flex-column sub-menu">
                            <li class="nav-item {{ active_class(['tenant.subdomain.shares.index']) }}">
                                <a class="nav-link"
                                    href="{{ route($prefix . '.shares.index', ['subdomain' => $subdomain]) }}">{{ __('قائمة العروض') }}</a>
                            </li>
                            <li class="nav-item {{ active_class(['tenant.subdomain.shares.create']) }}">
                                <a class="nav-link"
                                    href="{{ route($prefix . '.shares.create', ['subdomain' => $subdomain]) }}">{{ __('إنشاء عرض جديد') }}</a>
                            </li>
                        </ul>
                    </div>
                </li>
        ...
        @elseif($isAdminContext)
            ...
            <li class="nav-item {{ active_class(['admin.users.*', 'admin.roles.*', 'admin.permissions.*']) }}">
                <a class="nav-link" data-toggle="collapse" href="#admin-users-menu"
                    aria-expanded="{{ is_active_route(['admin.users.*', 'admin.roles.*', 'admin.permissions.*']) }}"
                    aria-controls="admin-users-menu">
                    <i class="menu-icon mdi mdi-account-multiple"></i>
                    <span class="menu-title">{{ __('app.users') }}</span>
                    <i class="menu-arrow"></i>
                </a>
                <div class="collapse {{ show_class(['admin.users.*', 'admin.roles.*', 'admin.permissions.*']) }}"
                    id="admin-users-menu">
                    <ul class="nav flex-column sub-menu">
                        ...
                        <li class="nav-item {{ active_class(['admin.permissions.*']) }}">
                            <a class="nav-link"
                                href="{{ route('admin.permissions.index') }}">{{ __('app.permissions') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
            @php
                $plansSummary = \App\Models\Tenant::select(
                    'Plan',
            ...
            <li class="nav-item {{ active_class(['admin.subscribers.*']) }}">
                <a class="nav-link" data-toggle="collapse" href="#admin-subscriptions-menu"
                    aria-expanded="{{ is_active_route(['admin.subscribers.*']) }}"
                    aria-controls="admin-subscriptions-menu">
                    <i class="menu-icon mdi mdi-receipt"></i>
                    <span class="menu-title">{{ __('app.subscriptions') }}</span>
                    <i class="menu-arrow"></i>
                </a>
                <div class="collapse {{ show_class(['admin.subscribers.*', 'admin.payments.*']) }}"
                    id="admin-subscriptions-menu">
                    <ul class="nav flex-column sub-menu">
                        <li class="nav-item {{ active_class(['admin.subscribers.index']) }}">
                            <a class="nav-link"
                                href="{{ route('admin.subscribers.index') }}">{{ __('app.view_all') }}</a>
                        </li>
                        <li class="nav-item {{ active_class(['admin.payments.*']) }}">
                            <a class="nav-link"
                                href="{{ route('admin.payments.index') }}">{{ __('app.payments') }}</a>
                        </li>
                        @foreach ($plansSummary as $plan)
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('admin.subscribers.index', ['plan' => $plan->Plan]) }}">
            ...
            <li class="nav-item {{ active_class(['admin.market.*']) }}">
                <a class="nav-link" data-toggle="collapse" href="#admin-market-menu"
                    aria-expanded="{{ is_active_route(['admin.market.*']) }}" aria-controls="admin-market-menu">
                    <i class="menu-icon mdi mdi-domain"></i>
                    <span class="menu-title">{{ __('السوق العقاري') }}</span>
                    <i class="menu-arrow"></i>
                </a>
                <div class="collapse {{ show_class(['admin.market.*']) }}" id="admin-market-menu">
                    <ul class="nav flex-column sub-menu">
                        <li class="nav-item {{ active_class(['admin.market.offers']) }}">
                            <a class="nav-link" href="{{ route('admin.market.offers') }}">{{ __('العروض') }}</a>
                        </li>
                        <li class="nav-item {{ active_class(['admin.market.operations']) }}">
                            <a class="nav-link"
                                href="{{ route('admin.market.operations') }}">{{ __('العمليات') }}</a>
                        </li>
                        <li class="nav-item {{ active_class(['admin.market.buyers']) }}">
                            <a class="nav-link" href="{{ route('admin.market.buyers') }}">{{ __('المشترون') }}</a>
                        </li>
                        <li class="nav-item {{ active_class(['admin.market.alerts']) }}">
                            <a class="nav-link" href="{{ route('admin.market.alerts') }}">{{ __('التنبيهات') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
            ...
            <li class="nav-item {{ active_class(['admin.reports.*']) }}">
                <a class="nav-link" data-toggle="collapse" href="#admin-reports-menu"
                    aria-expanded="{{ is_active_route(['admin.reports.*']) }}" aria-controls="admin-reports-menu">
            ...
            --}}
                        <li class="nav-item {{ active_class(['admin.reports.upcoming-expirations']) }}">
                            <a class="nav-link"
                                href="{{ route('admin.reports.upcoming-expirations') }}">{{ __('app.upcoming_expirations') }}</a>
                        </li>
                        <li class="nav-item {{ active_class(['admin.reports.plans']) }}">
                            <a class="nav-link"
                                href="{{ route('admin.reports.plans') }}">{{ __('app.plans_report') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
                ...
                <div class="nav-link">
                    <div class="user-wrapper">
                        <div class="profile-image">
                            <img src="{{ url('assets/images/faces/face8.jpg') }}"
                        ...
                        <div class="text-wrapper">
                            ...
                            <div class="dropdown" data-display="static">
                                <a href="#" class="nav-link d-flex user-switch-dropdown-toggler"
                                    id="UsersettingsDropdown" href="#" data-toggle="dropdown"
                                    aria-expanded="false">
                ...
                <div class="collapse {{ show_class(['basic-ui/*']) }}" id="basic-ui">
                    <ul class="nav flex-column sub-menu">
                        ...
                        <li class="nav-item {{ active_class(['basic-ui/typography']) }}">
                            <a class="nav-link"
                                href="{{ url('/basic-ui/typography') }}">{{ __('Typography') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
                ...
                <div class="collapse {{ show_class(['user-pages/*']) }}" id="user-pages">
                    <ul class="nav flex-column sub-menu">
                        ...
                        <li class="nav-item {{ active_class(['user-pages/lock-screen']) }}">
                            <a class="nav-link"
                                href="{{ url('/user-pages/lock-screen') }}">{{ __('Lock Screen') }}</a>
                        </li>
                    </ul>
                </div>
            </li>
