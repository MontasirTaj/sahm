        ...
        <div class="col-md-11 col-lg-11 col-xl-10 mx-auto">
            <div class="card">
                <div class="card-body">
                    ...
                    <form id="attachment-upload-form" method="POST"
                        action="{{ route('tenant.subdomain.attachments.store', ['subdomain' => $subdomain]) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <div class="custom-file-upload d-flex align-items-center border rounded bg-light p-2">
                                <button type="button" class="btn btn-outline-primary btn-sm me-2"
                                    onclick="document.getElementById('attachment').click()">{{ __('app.browse') }}</button>
                                <span id="attachment-label"
                                    class="text-muted small flex-grow-1">{{ __('app.no_file_selected') }}</span>
                            </div>
                            <input type="file" name="attachment" id="attachment"
                                class="d-none @error('attachment') is-invalid @enderror" required>
                            @error('attachment')
                        ...
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mt-4 mb-3">
                            ...
                            <div class="mt-3 mt-md-0">
                                <div class="btn-group btn-group-sm" role="group" aria-label="{{ __('app.export') }}">
                                    <a href="{{ route('tenant.subdomain.attachments.export.excel', ['subdomain' => $subdomain]) }}"
                                        class="btn btn-outline-success">
                                        <i class="mdi mdi-file-excel-outline"></i>
                                        <span>{{ __('app.export_excel') }}</span>
                                    </a>
                                    <a href="{{ route('tenant.subdomain.attachments.export.pdf', ['subdomain' => $subdomain]) }}"
                                ...
                                <tbody>
                                    @foreach ($attachments as $att)
                                        <tr>
                                            ...
                                            <td>{{ $att->type === 'image' ? __('app.attachment_type_image') : __('app.attachment_type_file') }}
                                            </td>
                                            <td>{{ $att->extension }}</td>
                                            <td>{{ $att->size_bytes ? number_format($att->size_bytes / 1024, 1) : '-' }}
                                            </td>
                                            <td>{{ $att->type === 'file' && $att->page_count ? $att->page_count : '-' }}
                                            </td>
                                            <td>{{ $att->created_at?->toDateString() }}</td>
                                            <td>
                                                <a href="{{ route('tenant.subdomain.attachments.edit', ['subdomain' => $subdomain, 'attachment' => $att->id]) }}"
                                                    class="btn btn-sm btn-outline-primary tenant-action-btn">
                                                    <i class="mdi mdi-pencil-outline"></i>
                                                    <span>{{ __('app.edit') }}</span>
                                                </a>
                                                <form method="POST"
                                                    action="{{ route('tenant.subdomain.attachments.destroy', ['subdomain' => $subdomain, 'attachment' => $att->id]) }}"
                                                    style="display:inline-block"
                                                    onsubmit="return confirm('{{ __('app.confirm_delete_attachment') }}');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="btn btn-sm btn-outline-danger tenant-action-btn">
...
@push('custom-scripts')
    <script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js"
        integrity="sha512-D0QDt4xQMBmQAs3YOzYqSY+fhbQEiyXMSOC+FsuZyl6wAVoVU/SS5xInROMM59ByY4zJIcYWsUTQCMYXAs3kPg=="
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fileInput.addEventListener('change', function() {
                ...
                if (!window['pdfjsLib'] || (file.type !== 'application/pdf' && !file.name.toLowerCase()
                        .endsWith('.pdf'))) {
                    return;
                }
